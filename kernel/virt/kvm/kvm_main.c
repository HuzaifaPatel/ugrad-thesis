...
// for ioctl calls. Prevents race condition effects
static unsigned long long num_kvm_vms;
static int sum_kvm_vcpus;
int* vcpu_count;
			  ...
			  unsigned int ioctl, unsigned long arg)
{
	...
	case KVM_GET_VM_SIZE:
		struct kvm* kvm_temp;
		mutex_lock(&kvm_lock);
		num_kvm_vms = 0;

		list_for_each_entry(kvm_temp, &vm_list, vm_list){
			num_kvm_vms++;
		}
		r = copy_to_user((void *)arg, &num_kvm_vms, sizeof(num_kvm_vms));
		mutex_unlock(&kvm_lock);
		break;


	case KVM_GET_VCPU_SIZE:
		struct kvm* kvm_temp_temp;
		int temp_kvm_counter = 0;
		vcpu_count = kmalloc(sizeof(vcpu_count) * num_kvm_vms, GFP_USER);
		sum_kvm_vcpus = 0;

		mutex_lock(&kvm_lock);
		list_for_each_entry(kvm_temp_temp, &vm_list, vm_list){
			if(temp_kvm_counter + 1 <= num_kvm_vms){
				vcpu_count[temp_kvm_counter++] = kvm_temp_temp->created_vcpus;
				sum_kvm_vcpus += kvm_temp_temp->created_vcpus;
			}
		}
	...
	case KVM_GET_VM_VCPU_PID:
		int* vm_vcpu_pid = kmalloc(sizeof(vm_vcpu_pid) * (num_kvm_vms + sum_kvm_vcpus), GFP_USER);
		int vm_vcpu_pid_counter = 0;
		int kvm_counter = 0;
		struct kvm* kvm_temp_temp_temp;
		struct kvm_vcpu* temp_vcpu;
		unsigned long index;

		// mutex_lock(&kvm_lock);
		list_for_each_entry(kvm_temp_temp_temp, &vm_list, vm_list){
			if(kvm_counter + 1 <= num_kvm_vms){
				vm_vcpu_pid[vm_vcpu_pid_counter++] = kvm_temp_temp_temp->userspace_pid;
				
				xa_for_each(&kvm_temp_temp_temp->vcpu_array, index, temp_vcpu){
					vm_vcpu_pid[vm_vcpu_pid_counter++] = pid_nr(temp_vcpu->pid);
				}
				kvm_counter++;
			}
