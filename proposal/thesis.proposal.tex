...
{\large
Hypervisors have been steadily rising in popularity, especially in the domain of cloud computing, where they play a key role in up-keeping server consolidation, workload balancing, and more. To maintain the stability of virtualized environments, reliable techniques for virtual machine anomaly detection and prevention are required.
\newline
}

{\large
\noindent One of the techniques for anomaly detection and prevention is an intrusion prevention system (IPS), a technique for monitoring and manipulating the runtime state of a VM for anomalies. Its usefulness has been proven by the vast number of tools proposed by both industry and academia. However, many existing prototypes are designed for guest-based deployment, which we argue is not an ideal solution due to the risk of guest OS tampering, and deployability issues.
\leavevmode\newline
}

{\large
\noindent For this reason, we decide to take a more holistic approach by proposing a hypervisor-based intrusion prevention system for the Intel VMX component of the Kernel Virtual Machine (KVM) hypervisor. With influence from existing IDS like \textit{Nitro}, and Somayaji's \textit{pH}, we propose a methodology to instrument guest system calls and process data from the hypervisor layer of KVM. We use the aforementioned data to establish normal profiles for each executed executable on the system. By comparing process behavior to the normal profile, we can identify and prevent abnormal activity in guest states. Our evaluation shows that we can trace KVM guest system calls from the hypervisor. It also shows that we can map each system call to its corresponding guest process (from the hypervisor). Thirdly, it shows us that we can prevent future anomalies by either terminating future executions of a process or the entire VM. Our benchmark tests indicate that the use of our IPS produces great overhead in the guest system compared to when the IPS is not active, which negatively impacts the usability of the guest.
\leavevmode\newline
}
...
{\large
\noindent The following is to illustrate the use of SYSCALL and SYSRET in relation to execution modes:
\newline
}
...
\begin{figure}[ht]
    \centering
    \tikzfig{figures/syscall_sysret_transition}
...
{\large
Intel Virtualization Extension (VT-X), also known as Intel Virtual Machine Extension (VMX) is a set of CPU extensions that drives modern virtualization applications (like KVM) on modern Intel CPUs. VMX was released on November 13, 2005 on two models of Pentium 4 (Model 662 and 672) ~\cite{intelvirtualizationlist}. As of 2015, almost all newer server, desktop and mobile Intel processors support it ~\cite{intelvirtualizationlist}. To maintain consistency throughout this thesis, we will use the abbreviation "VMX".
\newline
}
...
\begin{center}
\captionof{table}{Process Information with Nitro}
\begin{tabular}{c}
...
\begin{center}
\captionof{table}{Process Information with Nitro + LibVMI}
\begin{tabular}{c}
\begin{lstlisting}[caption={}, frame=none]
  {
    "event": {
...
\State    $Disable(SMAP)$
\State    $INS = Read(\%RIP)$
\If       {$(((INS >> (8*0)) \& 0xff) == 0x0f) \ \&\& \ (((INS >> (8*1)) \& 0xff) == 0x05)$}
\State    $RAX$ = Read(\%RAX)
\State    $Enable(SMAP)$
\State    \Return RAX
\EndIf
...
{\large
When our sequences of system calls algorithm detects a anomalous process, we either terminate the process, or the associated VM that is running the malicious process. In our user space interface, we will provide the option for users to choose from one of these two responses when the VMI detects an anomaly. 
\newline
}

\subsection{Terminating Anomalous Virtual Machine}

{\large
Terminating a VM that is running an anomalous process is trivial because both our VMI and the VMs are running on VMX root processes. For this reason, we can simply use the kill system call to terminate a KVM VM. In fact, Frail automates anomaly pevention by using the kill system to terminate a KVM VM. If a user wanted to manually terminate a KVM VM, one could also use the kill system call, or use the virtual machine manager software to terminate the virtual machine. 
\newline
}

\subsection{Terminating Anomalous Process}
...
\If {compare(SOS(binary), FRAIL\_SOS)} \Return True \\
\hskip3em \Return False
    \EndIf
\EndFor
\Until binary = NULL
\end{algorithmic}
...
\caption{Example of Binaries tested using Frail and Bash Script \\ (Full Listing Shown in Appendix)} \label{tab:long} \\
