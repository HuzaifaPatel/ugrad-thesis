...
Additionally, a concept of the Virtual Machine Control Structure (VMCS) is introduced. The VMCS is a structure that is present
in RAM and is responsible for state-management, communication and configuration between the hypervisor and the guest VM. It contains all the information needed to manage the new non-root mode. It can be accessed and modified through the new instructions VMREAD and VMWRITE. 


A hyperivosr maintains N VCMSs, where N is the product of the number of VMs running on the hypervisor and the number of VCPUs running on each VM. In other words, there exists one VMCS for each virtual processor of each virtual machine. However, only one VMCS is present on the physical processor at a time. The physical processor has a register pointing to the current VMCS, and all
VM instructions (such as VMLAUNCH) use the current VMCS. The VMM may use the VMPTRLD to load the address of a VMCS, making it current. The VMCS data structure has several fields, that may be grouped as follows:



Broadly speaking, the VMCS is a contiguous array of fields that can be grouped in four different categories.


When the VMX is enabled on a CPU, the CPU will enter non-root mode via the
new VMLAUNCH and VMRESUME instructions, and it returns to root mode for a
number of reasons, collectively called VM exits. VM exits should return control
to the VMM, which should complete the emulation of the action that the guest
code was trying to execute, then give control back to the guest by re-entering
non-root mode. All the new VM instructions are only allowed in root/system
mode.
% \newline


\begin{itemize}
    \item 1. Host-State: Stores part of the hypervisor state after a VMLAUNCH or VMRESUME.
    \item 2. Guest-State: Stores part of the guest state after a VM-exit.
    \item 3. Control: Determines VM-exit conditions as well as enabled virtualization features.
    \item 4. VM-exit information: Provides additional information for a VM-exit.
  \newline
\end{itemize}


{\large
While the VMCS is fundamentally a simple memory-mapped sequence of bytes, the
Intel manual states that it is for the most part an opaque data structure that is only to
be read/written from/to with the designated VMREAD and VMWRITE instructions [12]. To
illustrate the purpose of the VMCS, we will briefly describe a selection of fields located
in the VM-exit information part of the VMCS.



Exit Qualification: The exit qualification field provides additional information for
a specific VM-exit. The meaning of individual bits is specific to the respective VM-
exit. For example, the meaning of the exit qualification for the MOV DR VM-exit is
depicted in Table 2.1.
• Guest-Linear Address: For selected VM-exits, this field receives a linear address
with specific meaning to that VM-exit. For instance, VM-exits due to the execution
of the LMSW instruction with a memory operand receive the linear address of that
operand.
• Guest-Physical Address: For VM-exits due to an EPT violation or an EPT mis-
configuration, this field receives the guest-physical address that caused the EPT
violation or EPT misconfiguration.
• VM-Exit Instruction Length: For VM-exits resulting from the execution of a
specific set of instructions, this field receives the length of the instruction that
caused the VM-exit.
• VM-Exit Instruction Information: For VM-exits resulting from the execution of
a specific set of instructions, this field receives additional information about the
instruction that caused the VM-exit. For example, for the INS and OUTS instruc-
tions, it contains 3 bits specifying the address size as well as 3 bits specifying the
utilized segment register.
\newline
}
...
{\large
We now want to give an overview of the life cycle of a hypervisor executing on
VMX. First, a program executing in root mode needs to execute the VMXON instruction
to enable VMX. At this point, the program is considered a hypervisor. Second, the
hypervisor sets up a valid VMCS with the appropriate control bits set. Third, the
hypervisor can launch a VM with the VMLAUNCH instruction, transferring execution to
\newline
}
...
{\large
As mentioned, VM-exits transfer control back to the hypervisor and populate certain
fields in the VMCS to inform the hypervisor about what triggered the VM-exit. Certain
VM-exits occur unconditionally, while others occur conditionally (e.g., based on control
bits in the VMCS) One field that is populated for every VM-exit is the exit reason. It is
32 bits wide and mainly indicates the reason for the VM-exit. Currently, there are 69
...
{\large
To illustrate the concept of VM-exits in more detail, we will now take a look at an
example: the MOV DR VM-exit with exit code value 29. The Intel manual states that a
MOV DR VM-exit occurs whenever guest software attempts to MOV to or from a debug
register and the “MOV-DR exiting” VM-execution control was 1. In Figure 2.2 we can
see the hypervisor launching a VM in steps (1) and (2). Then, the instruction MOV DR0,
RCX attempts to overwrite the DR0 register in step (3), triggering a VM-exit (4) back into
the Hypervisor. The manual states that for this type of VM-exit, the exit qualification
field of the VMCS will be populated as displayed in Table 2.1. The hypervisor deals
with the VM-exit, taking into account the fields populated in the exit qualification, and
eventually performing a VMRESUME (5) to transfer control back to the guest VM.
\newline
}
