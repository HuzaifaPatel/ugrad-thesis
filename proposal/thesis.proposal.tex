...
{\large
Exceptions are type of signals from a device, such as a keyboard or a hard drive, to the CPU, telling it to immediately stop whatever it is currently doing due to an abnormal or unprecedented event that occured after the execution of a program.
...
However, we will only discuss relevant areas of faults and traps as they pertain to our VMI system's design and implementation.
\newline
}
...
\paragraph{Invalid Opcode Fault}

{\large
An illegal opcode, also called an unimplemented operation,[1] unintended opcode[2] or undocumented instruction, is an instruction to a CPU that is not mentioned in any official documentation released by the CPU's designer or manufacturer.
\newline
}

{\large
If a CPU detects a protection violation, it stops executing the code and sends a GPF interrupt to the kernel fault. In most cases, the operating system removes the failing process from the execution queue, signals the user, and continues executing other processes. If, however, the operating system fails to catch the GPF, i.e. another protection violation occurs before the operating system returns from the previous GPF interrupt, the CPU signals a double fault, stopping the operating system. If yet another failure (triple fault) occurs, the CPU is unable to recover, and enters a special halt state called "Shutdown", which can only be exited through a hardware reset.
\newline
}s

Protection rings use several techniques to achieve this separation.

First, the OS and all of its processes, functions, user applications, etc., are apportioned to a specific ring. This ring is the only place where these processes are permitted to execute. If a process needs other outside processes or resources, it must communicate within the protection ring construct as described below.


In reality, the protection rings are conceptual. Figure 5.2 shows an illustration of the protection ring schema.

It is designed to have a hierarchical design that separates and limits the interaction between the user space and kernel space within the OS.
